...
## ADR-017 — HTML parserはlxmlではなく標準ライブラリ`html.parser`を使う
...
`ARCHITECTURE.md` 12.1は"DOM parserは`lxml`を第一候補とする"と明記している。しかしTASK-G001(Safe HTML parser)実装時点で、本プロジェクトのランタイム依存はTASK-E002で追加した`zstandard`のみであり、`lxml`はネイティブ拡張を伴う比較的重い依存である。12.1が要求する性質("外部entity解決を無効にする"、"ネットワークアクセスを無効にする"、recovery mode)は、標準ライブラリの`html.parser.HTMLParser`であれば、そもそもネットワークI/Oや外部DTD/entity解決を行う機構自体を持たないため、追加設定なしに構造的に満たされる。

### Decision

- `src/wikiepwing/normalize/html_parser.py`は`lxml`ではなく標準ライブラリ`html.parser.HTMLParser`(`convert_charrefs=True`)を用いる。
- malformed HTMLの回復方針(`[normalize] html_recover`)、`max_dom_depth`超過時の切り捨て、未対応閉じタグ・未クローズタグの検出は自前のスタックベースDOM構築ロジックで実装する。

### Consequences

- 新規依存(`lxml`とそのビルド依存)を追加せずに済み、Dockerイメージサイズとビルド複雑度を抑えられる。
- 一方で、`lxml`が提供する高度なXPath/CSS selector機能は使えないため、TASK-G002以降のroot selection/node removal/table分類は自前の木構造に対する再帰探索で実装する必要がある(既にTASK-G002で実装済み)。
- 大規模・実際に近いHTML(表の複雑なネスト、名前空間付きXHTML断片等)に対する堅牢性は`lxml`の実績ある実装に劣る可能性がある。全件buildに進む前のGate(3/10/100/10,000記事)で問題が顕在化した場合は、本ADRを再検討し`lxml`への切り替えを行う。

### Alternatives

- `lxml`(`ARCHITECTURE.md`記載の第一候補): ネイティブ依存が増え、`network_access=False`や`resolve_entities=False`等を明示的に設定する必要があり誤設定のリスクがある。将来Gateで標準ライブラリの限界が明らかになれば採用を再検討する。
- `BeautifulSoup4` + `html.parser`: 依存が増える割に本プロジェクトが必要とする機能(型付きDOM木、diagnostics付き回復)を素朴なwrapper以上には提供しないため不採用。
