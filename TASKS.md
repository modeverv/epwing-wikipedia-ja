...
### TASK-T049 [x] Improve exact-match candidates, readings, and Lookup comparison

**依存:** T048

`日本`のLookup完全一致検索で括弧付き同名記事を複数の固有候補として返すため、括弧前の基底タイトルを共有headwordへ追加し、FreePWING登録時にEUC-JP変換後の同一キー・同一本文位置を除去する。導入部から安全に抽出できる読みを表示見出しへ付与し、`日本`・`にほん`・`にっぽん`・`Japan`についてexact/prefix/keywordの参照版比較とLookup.elの検索方式契約を自動テスト・文書へ固定する。
