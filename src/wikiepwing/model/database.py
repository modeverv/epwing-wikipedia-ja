"""Explicit SQLite migration and verification support for model.sqlite3.

Mirrors `wikiepwing.ingest.database` (raw.sqlite3's migration engine) for the
normalized-model database (`DATA_CONTRACTS.md` 5, "model.sqlite3 schema
draft"). The duplication is accepted for now, matching the raw/reference
database modules' precedent.
"""
...
class ModelDatabaseError(ValueError):
    """Raised when model schema migration or verification fails."""
...
def connect_model_database(path: Path) -> sqlite3.Connection:
    """Open a model database with required safety pragmas enabled."""
...
def _default_migrations_path() -> Path:
    candidates = (Path.cwd() / "migrations/model", Path("/app/migrations/model"))
    for candidate in candidates:
        if candidate.is_dir():
            return candidate
    raise ModelDatabaseError("model migration directory was not found")


def _load_migrations(directory: Path, max_bytes: int) -> tuple[Migration, ...]:
    if max_bytes < 1:
        raise ModelDatabaseError("max_migration_bytes must be positive")
    root = directory.expanduser().resolve(strict=True)
    if not root.is_dir():
        raise ModelDatabaseError(f"migration path must be a directory: {root}")
    migrations: list[Migration] = []
    for path in sorted(root.iterdir(), key=lambda item: item.name):
        if path.suffix != ".sql":
            continue
        if path.is_symlink():
            raise ModelDatabaseError(f"migration must not be a symlink: {path.name}")
        match = _MIGRATION_NAME.fullmatch(path.name)
        if match is None or not path.is_file():
            raise ModelDatabaseError(f"invalid migration filename: {path.name}")
        size = path.stat().st_size
        if size == 0 or size > max_bytes:
            raise ModelDatabaseError(
    ...
    if not migrations:
        raise ModelDatabaseError(f"no model migrations found in: {root}")
...
def _verify_applied_history(
    ...
    if len(applied) > len(migrations):
        raise ModelDatabaseError("database contains unknown model migrations")
...
def _verify_database(
    ...
    *,
    ...
    if foreign_key_errors:
        raise ModelDatabaseError(
            f"model database foreign key check found {len(foreign_key_errors)} errors"
        )
