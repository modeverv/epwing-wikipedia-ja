"""Explicit SQLite migration and verification support for reference observations."""
...
class ReferenceDatabaseError(ValueError):
    """Raised when reference schema migration or verification fails."""
...
def connect_reference_database(path: Path) -> sqlite3.Connection:
    """Open a reference database with required safety pragmas enabled."""
...
def _default_migrations_path() -> Path:
    candidates = (Path.cwd() / "migrations/reference", Path("/app/migrations/reference"))
    for candidate in candidates:
        if candidate.is_dir():
            return candidate
    raise ReferenceDatabaseError("reference migration directory was not found")


def _load_migrations(directory: Path, max_bytes: int) -> tuple[Migration, ...]:
    if max_bytes < 1:
        raise ReferenceDatabaseError("max_migration_bytes must be positive")
    root = directory.expanduser().resolve(strict=True)
    if not root.is_dir():
        raise ReferenceDatabaseError(f"migration path must be a directory: {root}")
    migrations: list[Migration] = []
    for path in sorted(root.iterdir(), key=lambda item: item.name):
        if path.suffix != ".sql":
            continue
        if path.is_symlink():
            raise ReferenceDatabaseError(f"migration must not be a symlink: {path.name}")
        match = _MIGRATION_NAME.fullmatch(path.name)
        if match is None or not path.is_file():
            raise ReferenceDatabaseError(f"invalid migration filename: {path.name}")
    ...
    if not migrations:
        raise ReferenceDatabaseError(f"no reference migrations found in: {root}")
...
def _verify_applied_history(
    ...
    if len(applied) > len(migrations):
        raise ReferenceDatabaseError("database contains unknown reference migrations")
...
"""
    try:
        ...
        if connection.in_transaction:
            connection.rollback()
        raise ReferenceDatabaseError(
            f"migration {migration.version:03d} failed: {error}"
        ) from error
...
def _verify_database(connection: sqlite3.Connection) -> None:
    ...
    if integrity is None or integrity[0] != "ok":
        ...
        raise ReferenceDatabaseError(f"reference database integrity check failed: {detail}")
    foreign_key_errors = connection.execute("PRAGMA foreign_key_check").fetchall()
    if foreign_key_errors:
        raise ReferenceDatabaseError(
            f"reference database foreign key check found {len(foreign_key_errors)} errors"
        )
