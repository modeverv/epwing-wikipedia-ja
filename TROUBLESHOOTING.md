...
### 2.3 ネイティブホスト実行時に`/data/...`が見つからない(`Read-only file system: '/data'`)

- **症状**: `acquire`等の実行時に`OSError: [Errno 30] Read-only file system: '/data'`や`FileNotFoundError`
- **原因**: `config/default.toml`の`[paths]`は`/data/...`をデフォルトにしており、これはDockerコンテナ内(`docker/app.Dockerfile`が`/data/*`を用意する)を前提としています。macOSホストでネイティブ実行(`uv run python -m wikiepwing.cli ...`)する場合、`/data`自体がホストに存在しません。
- **対処**: `[paths]`を実在するディレクトリに上書きする小さいTOMLを用意し、すべてのコマンドに`--config <上書きファイル>`を付けてください(README.mdの「実際のコマンド」§0参照)。相対パスはそのTOMLファイル自身のディレクトリからの相対で解決されるため、リポジトリ直下の`data/`を指すには`../data/...`と書く必要があります。
...
### 2.6 `verify`が`DUPLICATE_HEADWORD`を報告する
...
### 2.7 `image-fetch`が一部URLで400/404を返す
