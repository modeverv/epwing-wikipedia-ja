#!/usr/bin/env python3
"""Deterministically generate hundred_articles.ndjson (TASK-H012).

Synthetic data following the same Wikimedia Enterprise NDJSON schema as
tests/fixtures/enterprise/normal_articles.ndjson (TASK-D010), scaled to 100
articles for end-to-end testing (TASK-H013). Re-running this script produces
...
def generate() -> list[dict[str, object]]:
    """Return the 100 deterministic article records."""
    return [_build_article(index) for index in range(ARTICLE_COUNT)]


def main() -> None:
    """Write hundred_articles.ndjson next to this script."""
    destination = Path(__file__).parent / "hundred_articles.ndjson"
