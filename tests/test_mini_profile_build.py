...
MINI_PROFILE_CONFIG = Path("config/profiles/mini.toml")
...
def test_mini_profile_build_over_hundred_article_fixture(tmp_path: Path) -> None:
    # TASK-P002's acceptance gate: an end-to-end build using
    # config/profiles/mini.toml (register -> ingest -> normalize -> generate
    # -> verify) must complete cleanly, the same shape as TASK-H013's
    # 100-article gate. Wiring [images]/[math]/[tables]/[search]/
    # [references] config values into normalize/render behavior so a Mini
    # build actually differs from the default build (no images, math as
    # text, ...) is TASK-P004's job, not this test's.
    ...
    config = load_config(DEFAULT_CONFIG, [MINI_PROFILE_CONFIG])
    assert config.profile == "mini"
    ...
    with connect_model_database(model_database_path) as connection:
        ...
        assert media_count == 0, "Mini profile (images.enabled=false) must produce no images"
