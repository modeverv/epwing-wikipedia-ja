...
def test_lite_profile_build_over_hundred_article_fixture(tmp_path: Path) -> None:
    # TASK-P003's acceptance gate: an end-to-end build using
    # config/profiles/lite.toml (register -> ingest -> normalize -> generate
    # -> verify) must complete cleanly, the same shape as TASK-H013's
    # 100-article gate. Wiring [images]/[math]/[tables]/[search]/
    # [references] config values into normalize/render behavior so a Lite
